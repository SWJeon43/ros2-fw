...
from mpu9250.imusensor.MPU9250 import config
...
class MPU9250:
    ...
    def begin(self):
        ...
        self.setLowPassFilterFrequency("AccelLowPassFilter41")
        #self.setLowPassFilterFrequency("AccelLowPassFilter184")

        self.__writeRegister(self.cfg.SMPDivider, 0x00)
        #self.__writeRegister(self.cfg.SMPDivider, 0x04)     # 200 Hz 샘플링 레이트(ros2 타이머 주기보다 같거나 높아야함)
    ...
    def caliberateMagPrecise(self):
        ...
        # Check for NaN values in magvals(JSW)
        if np.isnan(magvals).any():
            print("NaN values detected in magvals")
            return

        centre, evecs, radii, v = self.__ellipsoid_fit(magvals)

        # Check for NaN values in radii(JSW)
        if np.isnan(radii).any():
            print("NaN values detected in radii")
            return
        ...
        if a <= 0 or b <= 0 or c <= 0:
            print(f"Invalid radii values: a={a}, b={b}, c={c}")
            return
